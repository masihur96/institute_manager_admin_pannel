...
class _DepositeDetailsState extends State<DepositeDetails> {
            ...
            ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  DateTime date = DateTime.fromMillisecondsSinceEpoch(int.parse(
                      firebaseProvider.depositHistoryList[index].date));
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                firebaseProvider
                                    .depositHistoryList[index].phone,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontSize: publicProvider.isWindows
                                      ? size.height * .02
                            ...
                            Expanded(
                              child: Text(
                                firebaseProvider
                                    .depositHistoryList[index].amount,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontSize: publicProvider.isWindows
                                      ? size.height * .02
                            ...
                            Expanded(
                              child: Text(
                                firebaseProvider
                                    .depositHistoryList[index].status,
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontSize: publicProvider.isWindows
                                      ? size.height * .02
  ...
  Future<void> _addDeposite(FirebaseProvider firebaseProvider) async {
    //  setState(() => _isLoading = true);
    Map<String, dynamic> map = {
      //  'depositPreBalance': addDepositTextController.text,
      'depositBalance': int.parse(firebaseProvider
              .userList[firebaseProvider.depositIndex].depositBalance) +
          int.parse(addDepositTextController.text),
    };
    await firebaseProvider
        .addDepositData(
            map, firebaseProvider.userList[firebaseProvider.depositIndex].id)
        .then((value) {
      if (value) {
        print('Success');
        // _emptyFildCreator();
  ...
  Future<void> _updateDeposit(FirebaseProvider firebaseProvider) async {
    //  setState(() => _isLoading = true);
    Map<String, dynamic> map = {
